...
''' <summary>
''' D3D_DpiContext 是新主版本控件迁移使用的非渲染 DPI 上下文。
''' 它不持有 GPU 对象，不创建 D3D/D2D 资源，也不替代任何 D3D_ 缓存类。
''' </summary>
Friend Structure D3D_DpiContext
    ...
    ''' </summary>
    Public Shared Function FromControl(control As Control) As D3D_DpiContext
        If control IsNot Nothing AndAlso Not control.IsDisposed Then
            Try
                If control.IsHandleCreated Then
                    ...
                    If dpi > 0 Then Return New D3D_DpiContext(dpi)
                End If
            Catch
            End Try

            Try
                If control.DeviceDpi > 0 Then Return New D3D_DpiContext(control.DeviceDpi)
            Catch
            End Try
        End If

        Return New D3D_DpiContext(96)
    End Function
End Structure
